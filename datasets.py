...
class GetMRI(Dataset):
    ...
    def __getitem__(self,index):
        cardiac_kt_data=loadmat(self.data_names[index])['data_kt_onet']
        weight=loadmat('./weight/weight1.mat')['weight']
        cardiac_kt_data_w = self.k2wgt(cardiac_kt_data,weight)
        kdata = np.zeros((192, 192, 2), dtype=np.float32)   
        kdata[:,:,0] = np.real(cardiac_kt_data_w)
        kdata[:,:,1] = np.imag(cardiac_kt_data_w)
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  dataset = GetMRI(root= "/home/lqg/桌面/kt-ncsn/dataset/data_k_t_onet")
  test_dataset = GetMRI(root= "/home/lqg/桌面/kt-ncsn/dataset/data_ztemp")
